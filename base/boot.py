...
def synchronizing_org(wait, driver, input_node:TreeNode):
    wait_click_xpath(wait, driver, xpath="(//input[@placeholder = '请选择字段项'])[7]")
...
def synchronizing_jg(wait, driver, input_node):
    #采集根组织信息
    root_text = 'root'
...
def recursion_jg(tree_root, wait, driver, root_node):
    org_items = []
    org_items = tree_root.find_elements(By.XPATH, ".//div[@role = 'treeitem']")
    for item in org_items:
        node_text = item.get_attribute('textContent')
        ...
        div_element = soup.find('div')
        if div_element and div_element.get('aria-disabled') == 'true':
            wait_click_xpath_relative(time_w = 0.5, element = item, xpath = ".//div[@class = 'el-tree-node__content']/span[@class = 'el-tree-node__expand-icon el-icon-caret-right']")
            new_tree_root = wait_return_subelement_relative(time_w = 0.5, element=item, xpath = ".//div[@role = 'group']")
